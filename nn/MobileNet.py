...
class MobileNetV3(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, return_features, get_net_outputs) -> Tensor:
        x = self.features(x)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)

        if return_features:
            if not get_net_outputs:
                return x
            else:
                features = x
...
class MobileNetV3_meta(nn.Module):
    def __init__(
        self,
        inverted_residual_setting: List[InvertedResidualConfig],
        last_channel: int,
        ...
        **kwargs: Any,
        ...
        self.reagents_num = reagents_num
    ...
    def _forward_impl(self, x: Tensor, reagent, return_features, get_net_outputs) -> Tensor:
        x = self.features(x)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
...
def _mobilenet_v3_conf(
    ...
    if arch == "mobilenet_v3_large" or arch == "mobilenet_v3_large_meta":
...
def _mobilenet_v3(
    ...
    **kwargs: Any,
    ...
    if architecture is not None and "meta" in architecture:
        ...
        model = MobileNetV3_EC(inverted_residual_setting, last_channel, **kwargs)
    else:
...
def mobilenet_v3_large(**kwargs: Any) -> MobileNetV3:
    ...
    return _mobilenet_v3(inverted_residual_setting, last_channel, "mobilenet_v3_large", **kwargs)


def mobilenet_v3_small(**kwargs: Any) -> MobileNetV3:
    inverted_residual_setting, last_channel = _mobilenet_v3_conf("mobilenet_v3_small", **kwargs)
    return _mobilenet_v3(inverted_residual_setting, last_channel, "mobilenet_v3_small", **kwargs)

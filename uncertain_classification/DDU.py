...
def calculate_density(test_img, net, meta_reagent, class_params_dict, 
    ...
    '''
    test_img_features, output = net(test_img, meta_reagent, return_features=True, get_net_outputs=True)
...
def get_trainloaders(test_config, net_id, nets_outputs):
    trainloaders = {}
    whole_train_size = 0
    markup_train_path = f"{test_config['path2nets_dir']}/{test_config['nets_names']}/{test_config['nets_names']}_{net_id}_markup_train.json"

    with open(markup_train_path, "r") as config_file:
    ...
    for class_label in [0, 1]:
        ...
        trainloaders[class_label] = get_datasetloader(test_config, markup_train_cur_class, run_mode=True)
...
def calc_epistemic_uncs_per_nets(path2outputs_train, net_id, test_config, n_components, densities, 
    ...
    with open(path2outputs_train, "r") as f:
    ...
    pca, transformed_features, features_mean_before, features_std_before = \
        get_pca(test_config, trainloaders, testloader, n_components, net, device, nets_outputs, net_id)
    ...
    with open(f"{path2save_dir}/per-net_epistemic_{n_components}PCAcomps_{test_set}_{net_id}.json", "w") as f:
        json.dump(epistemic_per_net, f, indent=4)


def calc_entropies_per_nets(test_on_val_data, path2outputs_train, path2outputs_test, test_config, net_id,
                            entropies, entropies_per_net, path2save_dir, test_set):
    if test_on_val_data:
        with open(path2outputs_train, "r") as f:
            nets_outputs = json.load(f)
    else:
        with open(path2outputs_test, "r") as f:
            nets_outputs = json.load(f)

    with open(test_config["path2test_markup"], "r") as f:
    ...
    with open(f"{path2save_dir}/per-net_entropies_{test_set}_{net_id}.json", "w") as f:
...
def prepare_paths(pca_components_num, path2save_dir, test_set):
    ...
    path2save_epist_uncs = f"{path2save_dir}/epistemic_uncs_{pca_components_num}PCAcomps_{test_set}.json"
...
def aggregate_and_save_epistemic_uncs(densities, path2save_epist_uncs):
    ...
    with open(path2save_epist_uncs, "w") as f:
...
def aggregate_and_save_entropies(entropies, path2save_entropies):
    ...
    with open(path2save_entropies, "w") as f:
        json.dump(ensemble_mean_alvs_names_dict, f, indent=4)



def DDU(test_config, path2save_dir, path2outputs_train, path2outputs_test=None, test_on_val_data=False, 
    calc_epistemic=True, calc_aleatoric=True, n_components=2, test_set="test"):
    ...
    for net_id in test_config["net_ids"]:
        ...
        if test_on_val_data:
            test_config["path2test_markup"] = \
            f"{test_config['path2nets_dir']}/{test_config['nets_names']}/{test_config['nets_names']}_{net_id}_markup_val.json"
        ...
        if calc_aleatoric:
            calc_entropies_per_nets(test_on_val_data, path2outputs_train, path2outputs_test, 
                test_config, net_id, entropies, entropies_per_net, path2save_dir, test_set)

    if calc_aleatoric:
        aggregate_and_save_entropies(entropies, path2save_entropies)

    if calc_epistemic:
        with open(path2save_densities, "w") as f:

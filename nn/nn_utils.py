...
# from nn.MobileNet import MobileNetV3
...
def get_datasetloader(config: Dict, data_markup: Dict, images: Dict = None, run_mode: bool = True,
    ...
    dataset = DatasetGenerator(config["path2dataset"], data_markup, images,
                               ...
                               augment=augment, random_crop=random_crop,
...
def get_net_architecture(net_arch_name):
    if "mobilenet" in net_arch_name:
        ...
        raise ValueError(f"Architecture {net_arch_name} is not supported.")
...
def save_ensemble_mistakes_imgs(path2ensemble_mistakes: Path, path2gt_markup: Path,
    ...
    for alv_name in mistakes["ensemble"]:
        ...
        alv_img = cv2.putText(alv_img, str(gt_markup[alv_name]["gt_result"]), (10, 100),
...
def estimate_experts_votes_prediction(path2predictions: Path, path2experts_markup: Path,
                                      path2dataset: Path, prediction_type: str = "std",
    ...
    for net_id, predicitons in predicitons_per_nets.items():
        ...
        print(f"Net #{net_id} has mae = {round(mae, 3)}, accuracy = {accuracy} "
              f"and pearson_correlation = {round(pearson_cor, 3)}")

    return accuracies, mae, pearson_cors


def experts_predictions_plot_heatmap(path2predictions: Path, path2gt_experts_markup: Path,
                                     net_ids=range(20), log_scale_mode=True, reagent="all",
                                     step=0.1, prediction_type="dist2integer", max_value=0.5):
    '''
        ...
        if reagent != "all":
            ...
            for net_id in net_ids:
                nets_predictions[str(net_id)] = {alv_name: alv_info for alv_name, alv_info
                                                 in nets_predictions[str(net_id)].items()
                                                 if alv_name in experts_markup}
    ...
    def aggregate_predictions(nets_predictions, net_ids):
        ...
        predicted_dict = {alv_name: np.mean(outputs) for alv_name, outputs
                          in predicted_dict.items()}
        return predicted_dict

    def calculate_heatmap_values(experts_markup, predicted_dict, step, max_value):
        stability_const = 1 / step
        max_stability_const = max_value * stability_const
        heatmap_data = [[0 for _ in range(int(max_stability_const) // int(step * stability_const))]
                        for _ in range(int(max_stability_const) // int(step * stability_const))]
    ...
    ax.set_xticks(np.arange(len(heatmap_data) + 1) - 0.5,
                  labels=np.round(np.arange(0, max_value + step, step), 1))
    ax.set_yticks(np.arange(len(heatmap_data) + 1) - 0.5,
                  labels=np.round(np.arange(0, max_value + step, step), 1))

    ax.set_title(f"Experts {prediction_type}: predicted VS gt (log scale of alvs numbers), "
                 f"reagent={reagent}")
    fig.colorbar(caxes)
...
if __name__ == "__main__":
    print(get_net_architecture("mobilenet_v3_small"))

...
class DatasetGenerator(Dataset):
    def __init__(self, root_dir, markup, images, transform, config, augment=False,
                 random_crop=False, experts_markup=None, reagents_list=None):
    ...
    def _initialize_augmentations(self, augment):
        ...
        if augment:
            ...
            if "noise" in self.config_aug:
                scale = self.config_aug["noise"][1:2]
            ...
            if "jigsaw" in self.config_aug:
                jigsaw = iaa.Sometimes(self.config_aug["jigsaw"]["probability"],
                                       iaa.Jigsaw(nb_rows=self.config_aug["jigsaw"]["x"],
                                                  nb_cols=self.config_aug["jigsaw"]["y"]))
    ...
    def _get_sector_mask(self, image, angle, offset):
        ...
        cond = (np.arctan2(mask_y - y_size/2, mask_x - x_size/2) * 180 / np.pi <= offset) | \
               (np.arctan2(mask_y - y_size/2, mask_x - x_size/2) *
                180 / np.pi >= right_border)
    ...
    def __getitem__(self, idx):
        ...
        if self.config_info.get("experts_mode", False):
            if (self.experts_markup is not None and
                    "expert_votes_positions" in self.experts_markup[img_name]):
                ...
                if self.config_info["experts_mode"] == "std":
                    experts_std = np.std(self.experts_markup[img_name]["expert_votes_positions"])
                    sample["experts_std"] = np.array(experts_std / 4, dtype=np.float32)
                elif self.config_info["experts_mode"] == "dist2integer":
                    sample["experts_dist2integer"] = np.array(0.5 - abs(0.5 - experts_mean),
                                                              dtype=np.float32)
                elif self.config_info["experts_mode"] == "entropy":
                    sample["experts_entropy"] = np.array(0 if experts_mean == 0 or experts_mean == 1
                                                         else -experts_mean * np.log(experts_mean) -
                                                         (1 - experts_mean) *
        ...
        if self.reagents_list is not None:
            ...
            list_index = self.reagents_list.index(self.markup_values[idx]["reagent"])
            sample["meta_reagent_type"][list_index] = 1.0

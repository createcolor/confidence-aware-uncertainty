...
def get_rc_info_averaging_mode(unc_alv_names_dicts: List[Dict], test_markup: Dict, \
        n_bins: int=20, reagent: str="all", misclassified_alvs=None, \
        ensemble_classifier_mode=False, path2save_markup: Path=None, net_ids=range(10)):
    '''
...
def get_accs_throwaway_rates(uncert_alv_name_dict: Dict, input_test_markup: Dict, \
    ...
    the earlier the alveolus is thrown away. Accuracies are calculated on the rest part of the test set. 
    ...
    sorted_uncertainty_rates = sorted(list(map(float, uncert_alv_name_dict_rounded.keys())), reverse=True)
...
def plot_rejection_curves(test_markup: Dict, misclas_alvs_names: List[str],
        uncertainty_types: Dict, plot_path: Path=None, title: str="", reagent="all"):
    '''
    ...
    for used_experts_num in range(1, 7):
        ...
        for used_experts in used_experts_ids:
            ...
            for dataset_part in dataset_parts:
                ...
                with open(path2misclas_alvs, "r", encoding='utf-8') as f:
                    misclassified_alvs = json.load(f)
                uncs_markup = get_rc_info_averaging_mode(unc_alv_names_dict, test_markup, 
                    misclassified_alvs=misclassified_alvs, path2save_markup=path2save_markup)
        ...
        aggregated_markup = {x: [np.mean(uncs[0]), np.mean(uncs[1])] for x, uncs in aggregated_markup.items()}
        with open(path2save_markup, "w", encoding='utf-8') as f:
...
def parse_args():
    ...
    parser.add_argument('-ds', '--dir2save', default=Path(
        "results/rejection_curves"), type=Path, help='Path to the directory where to save markups')
    parser.add_argument('-i', '--config', default=Path(
        "nn/rejection_curves_args.json"), type=Path, help='Path to the json file with uncertainty \
        estimation methods info')
    parser.add_argument('-nn', '--net_num', default=10, type=int, help='The number of nets in the ensemble')
    ...
    for method, info in config["UE_methods"].items():
        ...
        if info['single_net_file_name_template'] is None:
            ...
            for net_id in range(args.net_num):
                with open(f"{info['path2uncertainties']}/{info['single_net_file_name_template']}_{net_id}.json" , "r", encoding='utf-8') as f:
                    unc_alv_names_dicts.append(json.load(f))
        
        get_rc_info_averaging_mode(unc_alv_names_dicts, test_markup, n_bins=1000, \
            misclassified_alvs=misclassified_alvs, path2save_markup=path2save_reg_curves_info)

...
def calc_experts_uncertainty(path2markup: Path, path2save_markup: Path=None, \
    ...
    @param mode: sets the formula for the uncertainty, available modes are: 
        ...
        "entropy": entropy of experts for binary classes with the averaged experts vote as probability.
    '''
...
def aggregating_experts_and_nets_uncs(path2ensemble_outputs: Path, are_experts_unc_predicted: bool, \
    path2experts_markup: Path, path2save_markup: Path, experts_unc_mode: str="dist2integer", \
    ...
    @param weighting_coef: if aggregation_type is "sum", it is the weight of the ensemble uncertainty;
    '''
    ...
    ensemble_alvs_unc_dict = {alv_name: np.var(outputs_list) \
        for alv_name, outputs_list in ens_outputs.items()}
...
def convert_unc_alvs2alvs_unc(unc_alvs_markup: Dict) -> Dict:
    ...
    '''
    alvs_names_uncertainty_dict = {alv_name: float(uncertainty) \
        for uncertainty, alv_names in unc_alvs_markup.items() for alv_name in alv_names}
...
def parse_args():
    ...
    parser.add_argument('-em', '--experts_markup', type=Path, \
        default=Path("markup/experts_info_per_alvs.json"), \
        help='Path to the experts votes markup (per alvs)')
    
    parser.add_argument('-ep', '--experts_pred', type=Path, \
        default=Path("/alpha/korchagin/projects/expressto/nn/outputs_experts/MobileNet_small_10_10_80_meta_8_experts_average_700ep_outputs.json"), \
        help='Path to the experts votes markup (per alvs)')
    ...
    parser.add_argument('-ens', '--path2ensemble_outputs', \
        default=Path('/alpha/korchagin/projects/expressto/nn/outputs/MobileNet_small_10_10_80_meta_8_400ep_outputs.json'), \
        help="Path to the classifying ensemble outputs: {'net_0': {'alv_0': output_0, ...}, ...}")
    
    parser.add_argument('-mcmc', '--path2mcmc_outputs', \
        default=Path('results/mcmc_outputs'), \
        help="Path to the directory with classifying mcmc ensemble outputs: {'net_0': {'alv_0': output_0, ...}, ...}")
...
if __name__ == "__main__":
    ...
    for mode in ["TV"]:
        ...
        aggregating_experts_and_nets_uncs(args.path2ensemble_outputs,  \
            ...
            weighting_coef=0.5, \
            path2save_markup=args.path2save_markup_dir / path2save_markup)

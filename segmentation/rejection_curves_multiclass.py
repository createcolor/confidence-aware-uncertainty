...
if __name__ == "__main__":
    ...
    labels = F.one_hot(labels, num_classes=n_classes).movedim(-1, -2)
    ...
    models_predictions = onehot_argmax(models_predictions, n_classes=n_classes)
    ...
    with torch.no_grad():
        for cls in range(1, n_classes):
            ...
            for idx in range(len(models)):
                predictions_sorted = np.take_along_axis(models_predictions[idx, :, cls], sort_indices[idx], axis=1)
    ...
    if args.save_path is not None:
        ...
        for cls in range(1, n_classes):
            ...
            with open(args.save_path / f"rejection_curve_{cls}.json", 'w') as f:

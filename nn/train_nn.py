...
import nn.MobileNet
import nn.DenseNet
from nn.nn_utils import mkdirs, load_markup, get_net_architecture
...
def parse_args():
    ...
    parser.add_argument('-m', '--markup', default=Path("markup/BloodyWell/train_dataset_10_10_80.json"),
                        type=Path, help='Path to train dataset markup')
    parser.add_argument('-em', '--experts_markup', default=Path("markup/BloodyWell/experts_info_per_alvs.json"),
    ...
    if config["learn_from_scratch"]:
        if "pretrained epochs number" in config.keys():
...
def get_net_optimizer_lrshed(net, net_id, trained_ep_num, epoch_num2train, epochs_step,
                             config, args, net_dir, device, meta_learning_mode, use_SGLD = False):
    '''
    ...
    if epoch_num2train == epochs_step:  # at the start of the training loop
        if not config["learn_from_scratch"]:
            ...
            shutil.copyfile(
                f"{config['path2nets_dir']}/{net_dir2learn_from}/config_{net_dir2learn_from}.json", path2copy_config)
            ...
            reagents_num = 13 if ("meta" in config["architecture"] or "ec" in  config["architecture"]) else 0
...
if __name__ == "__main__":
    ...
    markup_trains, markup_vals = get_train_val_markups(
        nets_number, config, args.net_name, args.markup, net_dir, train_part=config.get("train_part", 0.5))
    ...
    for epoch_num2train in range(epochs_step, max(epochs, epochs // epochs_step) + 1, epochs_step):
        ...
        for net_id in range(nets_number):
            net, optimizer, lr_scheduler = get_net_optimizer_lrshed(net, net_id, trained_ep_num,
                                                                    epoch_num2train, epochs_step, config, 
                                                                    args, net_dir, device, meta_learning_mode, 
                                                                    use_SGLD)
            ...
            net, thresholds[str(net_id)] = train(device, 
                                                 ...
                                                 metalearning_mode=config.get("metalearning_mode", False),
        ...
        if not config["learn_from_scratch"]:
            ...
            config["last learning rates"][trained_ep_num] = lr_scheduler.get_last_lr()[
                0]
            with open(net_dir / f'config_{args.net_name}.json', "w") as f:
